...
public class InfoApi(PermissionOption options, MiscRepository info, IConfiguration configuration) : ControllerBase
{
	public async Task<ActionResult<ServerInfo>> GetInfo()
	{
		return Ok(
			new ServerInfo()
			{
				...
				PasswordLoginEnabled = !configuration.GetValue("AUTHENTICATION_DISABLE_PASSWORD_LOGIN", false),
				RegistrationEnabled = !configuration.GetValue("AUTHENTICATION_DISABLE_USER_REGISTRATION", false),
			}
		);
	}
}
